...
/* USER CODE BEGIN PFP */
int rand_1_to_6(void){
	return((rand() % 6)+1);
}
int Roll_Dice(void){
	int dice1 = rand_1_to_6();
	int dice2 = rand_1_to_6();
	int dice3 = rand_1_to_6();

	return(dice1*100 + dice2 * 20 + dice3);
}
/* USER CODE END PFP */
...
int main(void)
{
	...
	while (1)
	{
		...
		HAL_Delay(5000);
		//		uint8_t ledBinIndex = 0b11;
		//		for (uint8_t i = 1;i<=6;i++){
		//
		//			sct_uni_bar(ledBinIndex);
		//			HAL_Delay(delayXD);
		//			ledBinIndex <<= 1;
		//		}
		//		for (uint8_t i = 1;i<=6;i++){
